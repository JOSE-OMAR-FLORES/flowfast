    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-6">
            <div class="flex items-center gap-3 mb-2">
                ...
                <h2 class="text-2xl sm:text-3xl font-bold text-gray-900">💸 Registrar Gasto</h2>
            </div>
            <p class="text-sm text-gray-600 ml-9">Complete el formulario para registrar un nuevo gasto</p>
        </div>
        ...
        <form wire:submit.prevent="save">
            <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                ...
                <div class="p-4 sm:p-6 border-b border-gray-200">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select wire:model.live="beneficiary_id" 
                                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
                                <option value="">Seleccione beneficiario</option>
                                @foreach($beneficiaries as $beneficiary)
                                    <option value="{{ $beneficiary->id }}">
                                        {{ $beneficiary->name }} ({{ ucfirst($beneficiary->user_type) }})
                                    </option>
                                @endforeach
                            </select>
                            @error('beneficiary_id') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                ...
                {{-- Sección 2: Detalles del Gasto --}}
                <div class="p-4 sm:p-6 border-b border-gray-200 bg-gray-50">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Detalles del Gasto</h3>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            @error('expense_type') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <div class="relative">
                                ...
                                <input type="number" 
                                       ...
                                       min="0"
                                       placeholder="0.00"
                                       class="w-full pl-8 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
                            </div>
                        ...
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Fecha de Pago Programada</label>
                            <input type="date" 
                                   wire:model="due_date" 
                                   min="{{ date('Y-m-d') }}"
                                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent">
                        ...
                        {{-- Método de Pago --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Método de Pago Previsto</label>
                        ...
                        {{-- Referencia --}}
                        <div class="sm:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-2">Referencia o Número de Factura</label>
                            <input type="text" 
                ...
                {{-- Sección 3: Factura y Notas --}}
                <div class="p-4 sm:p-6">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Factura y Notas</h3>
                    
                    <div class="space-y-4">
                        {{-- Factura/Comprobante --}}
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Factura o Comprobante</label>
                            <div class="flex items-center justify-center w-full">
                                <label class="flex flex-col items-center justify-center w-full h-32 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 hover:bg-gray-100">
                                    ...
                                    <input type="file" wire:model="invoice" class="hidden" accept=".pdf,.jpg,.jpeg,.png">
                                </label>
                            </div>
                            @if ($invoice)
                                <div class="mt-2 text-sm text-green-600">Archivo seleccionado: {{ $invoice->getClientOriginalName() }}</div>
                            @endif
                            @error('invoice') <span class="text-red-500 text-xs mt-1">{{ $message }}</span> @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea wire:model="notes" 
                                      rows="3"
                                      placeholder="Notas o comentarios adicionales sobre el gasto..."
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-red-500 focus:border-transparent"></textarea>
                        ...
                        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1">
                                    <p class="text-sm font-medium text-blue-900">Proceso de Aprobación</p>
                                    <p class="text-sm text-blue-700 mt-1">
                                        Este gasto será registrado como <strong>Pendiente</strong> y requerirá aprobación del administrador antes de poder ser pagado.
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div class="px-4 sm:px-6 py-4 bg-gray-50 border-t border-gray-200 flex flex-col sm:flex-row gap-3 sm:justify-end">
                    ...
                    <button type="submit" 
                            ...
                            class="w-full sm:w-auto inline-flex justify-center items-center px-6 py-2.5 bg-red-600 hover:bg-red-700 text-white font-semibold rounded-lg transition-colors shadow-sm disabled:opacity-50">
